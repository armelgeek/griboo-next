 ...
 * Text Writing Module
 * 
 * Provides text writing animation functionality for the Griboo whiteboard engine.
 * This module handles handwriting-style text animation with typewriter and reveal effects.
 * Equivalent to the Python writing.py module.
 */
...
export class TextWritingAnimation {
  ...
  constructor(
    ...
    this.canvas.id = generateCanvasId('writing');
  ...
  private preRenderText(): void {
    ...
    if (!this.offscreenCanvas) {
      ...
      this.offscreenCanvas.id = generateCanvasId('writing-offscreen');
      ...
      if (!this.offscreenCtx) {
        ...
        tempCanvas.id = generateCanvasId('writing-temp');
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
      ...
      // Draw character by character with proper spacing to match the reveal
      for (let charIdx = 0; charIdx < line.length; charIdx++) {
        const char = line[charIdx];
        const charWidth = targetCtx.measureText(char).width;

        // Draw the character
        targetCtx.fillText(char, currentX, currentY);

        const startX = Math.floor(currentX);
        const endX = Math.ceil(currentX + charWidth);
