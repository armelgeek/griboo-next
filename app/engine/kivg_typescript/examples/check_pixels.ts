...
async function checkFramePixels() {
    const framePath = path.join(process.cwd(), 'output', 'debug_flash', 'frame_000.png');
    ...
    if (nonWhitePixels > 0) {
        console.log('🚨 FLASH DETECTED! The first frame is not empty.');
    } else {
        console.log('✅ First frame is empty.');
    }
}
